          ...
          <div style={{ marginTop: '1rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', lineHeight: '1.4' }}>
              ...
              1. Glissez ou enregistrez ce lien dans vos favoris : <a 
                href="javascript:(function(){let recipe=null;document.querySelectorAll('script[type=%22application/ld%2Bjson%22]').forEach(script=>{try{const data=JSON.parse(script.innerText);const findRecipe=(obj)=>{if(!obj)return null;if(obj['@type']==='Recipe')return obj;if(Array.isArray(obj)){for(let item of obj){const r=findRecipe(item);if(r)return r;}}if(typeof obj==='object'){if(obj['@graph'])return findRecipe(obj['@graph']);for(let k in obj){const r=findRecipe(obj[k]);if(r)return r;}}return null;};const r=findRecipe(data);if(r)recipe=r;}catch(e){}});if(!recipe||!recipe.recipeIngredient||recipe.recipeIngredient.length===0){const title=document.querySelector('.recipe-card__title')?.innerText.trim()||document.querySelector('h1')?.innerText.trim()||document.title;const imageUrl=document.querySelector('.recipe-card__image-container img')?.src||document.querySelector('.recipe-card__image')?.src||'';const prepTimeEl=document.querySelector('.core-ingredients__meta-prep-time .core-ingredients__meta-value')||document.querySelector('.core-ingredients__meta-prep-time');const prepTime=prepTimeEl?parseInt(prepTimeEl.innerText):15;const totalTimeEl=document.querySelector('.core-ingredients__meta-total-time .core-ingredients__meta-value')||document.querySelector('.core-ingredients__meta-total-time');const cookTime=totalTimeEl?Math.max(0,parseInt(totalTimeEl.innerText)-prepTime):30;const servingsEl=document.querySelector('#portions-count')||document.querySelector('.core-ingredients__meta-portions');const servings=servingsEl?parseInt(servingsEl.value||servingsEl.innerText):4;const ingredients=[];document.querySelectorAll('#ingredients li, .core-ingredients__ingredients-list li, .ingredients-section li').forEach(li=>{const text=li.innerText.trim();if(text)ingredients.push(text);});const instructions=[];document.querySelectorAll('#preparation-steps li, .preparation-steps li, .preparation-steps__step-content').forEach(li=>{const text=li.innerText.trim();if(text)instructions.push(text);});if(ingredients.length>0){recipe={name:title,description:'',prepTime,cookTime,recipeYield:servings,recipeIngredient:ingredients,recipeInstructions:instructions,image:imageUrl};}}if(!recipe){alert(%22Impossible d'extraire la recette. Assurez-vous d'e-tre sur une page de recette.%22);return;}const parseDuration=(iso)=>{if(!iso)return 0;if(typeof iso==='number')return iso;const match=iso.match(/PT(?:(\\d+)H)?(?:(\\d+)M)?/);if(!match)return 0;const hours=parseInt(match[1])||0;const minutes=parseInt(match[2])||0;return hours*60%2Bminutes;};let servings=4;if(recipe.recipeYield){const yieldStr=Array.isArray(recipe.recipeYield)?recipe.recipeYield[0]:String(recipe.recipeYield);const match=yieldStr.match(/\\d%2B/);if(match)servings=parseInt(match[0]);}const ingredients=(recipe.recipeIngredient||[]).map(ing=>({name:ing,quantity:1,unit:'pi%C3%A8ce'}));const instructions=[];const steps=recipe.recipeInstructions||[];if(Array.isArray(steps)){steps.forEach(step=>{if(typeof step==='string'){instructions.push(step);}else if(step.text){instructions.push(step.text);}else if(step['@type']==='HowToStep'){instructions.push(step.text);}else if(Array.isArray(step.itemListElement)){step.itemListElement.forEach(sub=>{if(sub.text)instructions.push(sub.text);});}});}else if(typeof steps==='string'){instructions.push(steps);}let imageUrl='';if(recipe.image){imageUrl=Array.isArray(recipe.image)?recipe.image[0]:(typeof recipe.image==='object'?recipe.image.url:recipe.image);}const importData={title:recipe.name||document.title,description:recipe.description||'',prepTime:parseDuration(recipe.prepTime)||15,cookTime:parseDuration(recipe.cookTime)||30,servings:servings,ingredients:ingredients,instructions:instructions,imageUrl:imageUrl};const url='http://localhost:3000/recipe/new?import='%2BencodeURIComponent(JSON.stringify(importData));window.location.href=url;})();"
